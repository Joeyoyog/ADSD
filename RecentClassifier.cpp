...
#include "./svs_packed.h"
#include "./bias.h"
#include "./alphas.h"
#include "./sv_norms_padded.h"
#include "ap_int.h"
#include "Exp.h"
...
void compute_class(ap_fixed<8,7> x_local[IMG_SIZE], ap_fixed<24,14> x_norm_in, ap_fixed<32,16> &result) {
    ...
    classify_label2: for (int i = 0; i < 165; i += 16) {
        ...
        // Packed dot products: 49 groups of 16 pixels
        classify_label1: for (int jg = 0; jg < SVS_GROUPS; jg++) {
            #pragma HLS PIPELINE II=1

            // Load 16 pixels from x_local for this group (broadcast)
            ap_fixed<8,7> xpix[16];
            #pragma HLS ARRAY_PARTITION variable=xpix complete dim=1
            for (int p = 0; p < 16; p++) {
                #pragma HLS UNROLL
                xpix[p] = x_local[jg*16 + p];
            }

            // Update 16 dot products in parallel (SV lanes)
            for (int k = 0; k < 16; k++) {
                #pragma HLS UNROLL

                sv_pack_t w = svs_packed[i+k][jg];

                ap_fixed<32,16> sum16 = 0;
                for (int p = 0; p < 16; p++) {
                    #pragma HLS UNROLL
                    ap_int<8> raw = (ap_int<8>)w.range(8*p + 7, 8*p);

                    ap_fixed<8,7> xi;
                    xi.range(7,0) = raw;        // reinterpret packed raw bits as ap_fixed<8,7>

                    ap_fixed<16,14> prod = xi * xpix[p];
                    sum16 += prod;
                }

                dot_products[k] += sum16;
            }
        }
...
void classify(hls::stream<axis_t> &in_stream,
	...
	#pragma HLS RESOURCE variable=svs_packed core=ROM_1P_BRAM //============LAST THING I WAS TRYING===================================

    // OPTIMIZATIONS
	//#pragma HLS ARRAY_PARTITION variable=svs_packed cyclic factor=16 dim=1
