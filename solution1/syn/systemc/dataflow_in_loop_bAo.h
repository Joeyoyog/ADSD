...
#ifndef _dataflow_in_loop_bAo_H_
#define _dataflow_in_loop_bAo_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "dataflow_in_loop_bAo_memcore.h"

SC_MODULE(dataflow_in_loop_bAo) {
...
protected:
    ...
    // buffer instances
    dataflow_in_loop_bAo_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(dataflow_in_loop_bAo) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "dataflow_in_loop_bAo_memcore_U_";
            name += szTmp;

            buffer[i] = new dataflow_in_loop_bAo_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "dataflow_in_loop_bAo.trace");
            // ports
